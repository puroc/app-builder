    ...
    <el-dialog title="添加用户" :visible.sync="addDialogFormVisible" top="5vh">
      ...
      <div>
        <el-form :model="addUserModel" :rules="validateUserRules" ref="addUserForm">
          ...
          <el-form-item label="角色" :label-width="formLabelWidth" prop='bindedRoles'>
            <el-checkbox-group v-model="addUserModel.bindedRoles">
              <el-checkbox v-for="item in this.rolesOfCurrentOrg" :label="item.id" :key="item.id">{{item.name}}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </el-form>
      </div>
